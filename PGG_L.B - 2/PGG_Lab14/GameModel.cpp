...
void GameModel::Update(float deltaTs, float mx, float my, float mz)
{
...
void GameModel::setForce(int index, float X, float Y, float Z)
{
	force[index].x = X;
	force[index].y = Y;
	force[index].z = Z;
...
{        
	for (int i = 0; i < size; i++)
	{
		...
		{
			force[i].x += 0.5f; // If so add to the X force of the particle
		}
